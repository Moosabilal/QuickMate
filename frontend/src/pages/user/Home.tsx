...
import { categoryService } from '../../services/categoryService'; // Make sure this path is correct
import { ICategoryResponse } from '../../types/category'; // Import your types
...
// Dummy Data for sections not pulled from categories for now
...
const Home = () => {
    ...
    // State for derived services (popular and trending)
    ...
    useEffect(() => {
        const loadCategories = async () => {
            ...
            try {
                ...
                // --- Derive Popular and Trending Services from fetched subcategories using LIFO ---
                const allSubCategories: ICategoryResponse[] = [];
                response.forEach(cat => {
                  console.log('this is category:', cat);
                  console.log('this is subcategories:', cat.subCategories);
                ...
                // Apply LIFO logic: reverse the collected subcategories so the latest are at the front
                const lifoSubCategories = [...allSubCategories].reverse();

                const numPopular = 4; // Number of popular services to display
                const numTrending = 4; // Number of trending services to display

                // Take the first 'numPopular' items from the LIFO array for popular services
                setPopularServices(lifoSubCategories.slice(0, numPopular));
                // Take the next 'numTrending' items for trending services
    ...
    return (
        <div className="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen">
            ...
            <main className="pt-20"> {/* Padding top to account for fixed header */}
                        ...
                        {fetchedCategories.map((category) => (
                            <Link to={`/category/${category._id}`} key={category._id} className="flex flex-col items-center p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition duration-200 transform hover:-translate-y-1">
                                <div className="p-4 bg-blue-100 dark:bg-blue-900 rounded-full mb-4">
                                    {/* Use category.icon for the image */}
                                    <img src={category.iconUrl || 'https://via.placeholder.com/64?text=Category'} alt={category.name} className='rounded-full w-16 h-16 object-cover' />
                                </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {popularServices.map((service) => (
                                <Link to={`/service/${service._id}`} key={service._id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-200 transform hover:-translate-y-1">
                                    {/* Use service.icon for the image */}
                                    ...
                                    <div className="p-5">
                                        ...
                                        {/* Assuming a price field might be part of ISubCategoryResponse or fetched separately */}
                ...
                {/* Featured Providers Section (remains unchanged as it uses dummy data) */}
                <section className="bg-gray-100 dark:bg-gray-950 px-4 py-12 md:py-16">
                    <div className="container mx-auto">
                ...
                {/* What People Are Saying (Testimonials - remains unchanged) */}
                <section className="container mx-auto px-4 py-12 md:py-16">
